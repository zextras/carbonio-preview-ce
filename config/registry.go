...
func registerCEKeys() {
	...
	application := []KeyEntry{
		...
		// Credentials (db-name, db-username, db-password) are read from Consul KV
		// at runtime inside config.Load() and are NOT registered as static keys
		// because their values are secret and generated by carbonio-preview-db-bootstrap.
		{
