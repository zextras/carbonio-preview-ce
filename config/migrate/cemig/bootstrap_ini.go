...
// the framework but does NOT import this package — never inherits CE's
// bootstrap or migrations.
package cemig

import "github.com/zextras/carbonio-preview-ce/v3/config/migrate"

func init() {
	if err := migrate.RegisterBootstrap("ce", CEBootstrap()); err != nil {
		panic("cemig: register bootstrap into set \"ce\": " + err.Error())
	}
}

// CEBootstrap returns the CE one-time bootstrap that reads entries from the
// legacy Python config.ini and writes them into config.properties
// (networking), Consul KV (application), and a systemd drop-in file
// (DropInEnvEntries).  It is the one-time legacy Python→Go import, run once
// (per --setup invocation) before the "ce" set's Vn KV→KV migration series.
//
...
//
// Keys in the ADVANCED bootstrap territory (nginx lookup / memcached) are
// deliberately absent from both maps; they survive in the ini until the
// advanced binary runs its own bootstrap (registered into the "advanced" set,
// entirely separate from this "ce" set).
func CEBootstrap() migrate.Bootstrap {
