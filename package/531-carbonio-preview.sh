...
# Reload systemd unit files so any log-level drop-in written by --setup above
# (at /etc/systemd/system/carbonio-preview.service.d/log-level.conf) is picked
# up before the wrapper's later service restart.
